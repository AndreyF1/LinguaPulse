...
const TranscriptDisplay: React.FC<{ history: TranscriptEntry[] }> = ({ history }) => {
    ...
    return (
        <div className="w-full h-48 bg-slate-900/50 border border-slate-700 rounded-lg p-3 overflow-y-auto mt-6 space-y-3 text-left">
            {history.map((entry, index) => (
                <div key={index} className={`flex ${entry.speaker === 'User' ? 'justify-end' : 'justify-start'}`}>
                    <div className={`px-3 py-2 rounded-xl max-w-[85%] break-words ${entry.speaker === 'User' ? 'bg-violet-600' : 'bg-slate-700'}`}>
            ...
            <div className="w-full max-w-md text-center">
                <div className="relative w-48 h-48 mx-auto flex items-center justify-center mb-8">
                    <svg className="absolute inset-0 w-full h-full" viewBox="0 0 100 100">
                        <circle className="text-slate-800" strokeWidth="5" stroke="currentColor" fill="transparent" r="45" cx="50" cy="50" />
                        <circle
                    ...
                    <MicrophoneIcon className={`h-16 w-16 text-white transition-all duration-300 ${status === 'Connected' ? 'text-violet-400 animate-pulse shadow-lg shadow-violet-500/50' : ''}`} />
                </div>
