...
import type { Answers, Question, Hook } from '../types';
...
const HookMicroDemo: React.FC = () => {
    ...
    const getDisplayContent = () => {
        if (aiSpeechText) {
            return (
                <div className="bg-slate-800 text-sm text-slate-200 p-3 rounded-lg shadow-md relative animate-fade-in w-full max-w-sm mx-auto">
                    <p>{aiSpeechText}</p>
        ...
        if (demoStep === 'done') {
            return (
                <div className="flex flex-col items-center text-center gap-2 animate-fade-in">
                    ...
                    <p className="text-slate-200 font-medium text-lg">Отлично! Демо завершено.</p>
                    <p className="text-slate-400 text-sm">Нажмите «Продолжить», чтобы двигаться дальше.</p>
                </div>
            );
        }
        ...
        return (
            <>
                ...
                <p className="bg-slate-900/70 font-mono text-violet-300 p-3 rounded-md text-lg transition-opacity duration-500 w-full max-w-sm mx-auto">
                    {promptText}
                </p>
            </>
        );
    ...
    if (demoStep === 'done') {
        ...
        if (recordingStatus === 'recording') {
            ...
            buttonClassName = 'bg-slate-500 cursor-wait';
        } else {
    ...
    return (
        <div className="bg-slate-700/50 border border-slate-600 text-white p-6 rounded-lg shadow-lg text-center animate-fade-in space-y-4">
...
const HookReviews: React.FC = () => (
    <div className="bg-slate-800/90 border border-slate-700 p-6 rounded-lg shadow-sm animate-fade-in text-center">
        <h3 className="text-xl font-bold text-slate-100 mb-2">«20 часов практики ломают языковой барьер»</h3>
        <p className="text-sm text-slate-400 mb-6 max-w-md mx-auto">
            Это не магия, а доказанный факт. Регулярные короткие сессии эффективнее редких длинных уроков.
        </p>
        <div className="space-y-4 text-left">
            <div className="bg-slate-700/50 p-4 rounded-lg flex items-start gap-4 border border-slate-600">
                <div className="flex-shrink-0">
                    <div className="w-12 h-12 bg-violet-500 rounded-full flex items-center justify-center font-bold text-white text-xl shadow-md">
                        А
                    </div>
                </div>
                <div>
                    ...
                    <p className="mt-2 text-sm font-semibold text-slate-300 text-right">– Анна, 32</p>
                </div>
            </div>
            ...
            <div className="bg-slate-700/50 p-4 rounded-lg flex items-start gap-4 border border-slate-600">
                ...
                <div>
                    ...
                    <p className="mt-2 text-sm font-semibold text-slate-300 text-right">– Марк, 28</p>
                </div>
            </div>
        </div>
    </div>
...
const HookFeedback: React.FC = () => {
    ...
    return (
    <div className="bg-slate-800/90 border border-slate-700 p-6 rounded-lg shadow-sm animate-fade-in">
        <p className="font-bold text-lg mb-2 text-center text-slate-100">Ваш прогресс — наглядно</p>
        <p className="text-sm text-slate-400 text-center mb-4">После каждой сессии вы получаете не просто оценку, а детальный разбор с рекомендациями.</p>
        
        <RadarChart data={radarData} theme="dark" />

        <div className="bg-slate-700/50 p-4 rounded-md">
            <h3 className="font-semibold text-slate-300 mb-2">Пример анализа (Словарный запас):</h3>
            <p className="text-sm text-slate-400">
                Вы уверенно используете базовую лексику. Чтобы сделать речь богаче, попробуйте добавлять синонимы. Например, вместо <code className="bg-slate-600 text-xs px-1 rounded">"good"</code> можно использовать <code className="bg-slate-600 text-xs px-1 rounded">"great"</code>, <code className="bg-slate-600 text-xs px-1 rounded">"excellent"</code> или <code className="bg-slate-600 text-xs px-1 rounded">"wonderful"</code>.
            </p>
        ...
        <p className="text-sm text-slate-400 text-center mt-6">В полном отчёте вы увидите такой же детальный разбор по <span className="font-semibold text-slate-300">каждому</span> из пяти аспектов языка.</p>
    </div>
    );
};
...
const Funnel: React.FC<FunnelProps> = ({ onComplete }) => {
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-slate-900 p-4 sm:p-6">
            <div className="w-full max-w-2xl bg-slate-800/50 backdrop-blur-sm border border-slate-700 rounded-2xl shadow-xl p-6 sm:p-10 transition-all duration-300">
                <div className="mb-6">
                ...
                {showingHook ? (
                    ...
                    currentQuestion && <>
                        <h2 className="text-2xl sm:text-3xl font-bold text-slate-100 mb-8 text-center">{currentQuestion.text}</h2>
                        <div className="space-y-3 mb-8">
                            {currentQuestion.options.map((option) => {
                                ...
                                return (
                                <label key={option} className="flex items-center p-4 border-2 border-slate-700 rounded-lg cursor-pointer hover:bg-slate-700/50 hover:border-violet-500 transition-all has-[:checked]:bg-violet-900/30 has-[:checked]:border-violet-500">
                                    <input
                                        ...
                                        className="h-5 w-5 text-violet-500 focus:ring-violet-500 border-slate-600 rounded bg-slate-800"
                                    />
                                    <span className="ml-4 text-md font-medium text-slate-200">{option}</span>
                                </label>
                                );
                            })}
                        </div>
                ...
                <div className="flex justify-between items-center mt-10">
                    <button
                        ...
                        className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-slate-300 bg-slate-700 rounded-lg hover:bg-slate-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                    >
                        <ChevronLeftIcon />
                        Назад
                    </button>
                    <button
                        ...
                        className="flex items-center gap-2 px-6 py-3 text-base font-semibold text-white bg-violet-600 rounded-lg shadow-md hover:bg-violet-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-slate-800 focus:ring-violet-500 disabled:opacity-50 disabled:bg-slate-600 disabled:cursor-not-allowed transition-all"
                    >
